...
def run_dashboard() -> None:
    ...
    for idx, step in enumerate(start_here_steps, start=1):
    ...
    if pd is not None and resource_items:
        st.markdown("**Derived resource inventory**")
    ...
    if review_index_resource is not None:
        st.markdown("**Review index preview**")
    ...
    if pipeline_summary_resource is not None:
        st.markdown("**Pipeline summary preview**")
    ...
    if ai_triage or ai_findings or ai_questions:
        st.markdown("**AI review quick view**")
    ...
    if artifact_priority_path is not None:
        ...
        st.markdown("**AI artifact priority**")
    ...
    st.caption("Advanced inspection surfaces start here. Use this lower section when the core review surfaces are no longer enough.")
    ...
    # Spotlight meta
    st.subheader("Spotlight metadata")
    ...
    # Spotlight events filter
    st.subheader("Spotlight event filters")
    ...
    # Fragment graph
    st.subheader("Fragment relationship graph")
    ...
    st.subheader("Recovery source comparison")
    ...
    st.caption("Optional future uplift: semantic search can be layered on later without changing the current local-first review contract.")
